...
设 $x_+$ 和 $x_-$ 分别是正、负边界超平面上的点，间隔宽度 $d$ 等于向量 $(x_+ - x_-)$ 与单位法向量的点积的绝对值：

$$d = \left| (x_+ - x_-)^T \frac{w}{\|w\|} \right|$$

将点积展开：

$$d = \frac{|w^T x_+ - w^T x_-|}{\|w\|}$$

由边界条件 $w^T x_+ + b = 1$ 和 $w^T x_- + b = -1$，可得 $w^T x_+ = 1 - b$，$w^T x_- = -1 - b$，代入：

$$d = \frac{|(1 - b) - (-1 - b)|}{\|w\|} = \frac{|1 - b + 1 + b|}{\|w\|} = \frac{2}{\|w\|}$$

**总结：** 经过上面的推导，SVM 两边界之间的间隔宽度就是 $\frac{2}{\|w\|}$。这也是为什么为了最大化间隔（Maximize Margin），我们需要最小化 $\|w\|$（为了数学求解方便，通常写成最小化 $\frac{1}{2}\|w\|^2$）。这就把一个寻找最大间隔的几何问题，漂亮地转化为了一个凸优化问题。

**直觉理解：**
- **为什么相减：** $x_+ - x_-$ 是为了在两个边界平面之间构造一个连接两面的向量（”搭一座桥”）。
- **相减后怎么用：** 因为这座桥通常是斜的，所以把它和单位法向量 $\frac{w}{\|w\|}$ 做点积运算，相当于把斜桥投影成垂直距离，从而算出间隔宽度 $\frac{2}{\|w\|}$。

### 补充：什么时候需要法向量求距离？

这里隐藏着一个经常容易被混淆的概念。**求二维空间里"两个点"之间的距离，是不需要（也不能）用法向量的。**

#### 情况一：两点之间的距离 — 直接用勾股定理

如果只有两个孤立的点 $A(x_1, y_1)$ 和 $B(x_2, y_2)$，没有"线"也没有"面"，自然不存在法向量。直接用欧几里得距离公式：

$$d = \sqrt{(x_2 - x_1)^2 + (y_2 - y_1)^2}$$

在向量里，这就相当于求向量 $\vec{AB}$ 的模长：$d = \|B - A\|$。

#### 情况二：为什么 SVM 里用到了法向量？

在 SVM 的推导中，虽然取了 $x_+$（正边界上的点）和 $x_-$（负边界上的点），但**根本目的并不是求 $x_+$ 和 $x_-$ 这两个点之间的距离**！

如果求这两点的距离，那就是 $\|x_+ - x_-\|$，这根"斜拉的绳子"对我们没用。

我们的**真实目的是求两面平行的"墙"（正边界和负边界）之间的垂直距离**。正因为涉及到了"墙"（直线/超平面），法向量才派上用场——它提供了一个"绝对垂直的方向标"，帮我们把斜拉的绳子投影成墙与墙之间的垂直宽度。

#### 补充：点到直线的距离

这是二维空间中最常用到法向量求距离的场景。

假设有一个点 $P(x_0, y_0)$ 和一条直线 $L: Ax + By + C = 0$，这条直线的法向量就是 $\vec{n} = (A, B)$。

**推导逻辑（和 SVM 非常像）：**

1. 在直线 $L$ 上随便找一个点 $Q(x_1, y_1)$
2. 把 $Q$ 和 $P$ 连起来，得到向量 $\vec{QP} = (x_0 - x_1, y_0 - y_1)$（相当于拉了一根斜的绳子）
3. 把这根斜绳子 $\vec{QP}$ 投影到法向量 $\vec{n}$ 的方向上

**第三步的投影具体怎么算？**

$\vec{QP}$ 是斜的，但我们只关心它在**垂直于直线方向**（即法向量方向）上的分量，那就是距离。向量点积除以模长正好就是"在某个方向上的投影长度"：

$$d = \frac{|\vec{QP} \cdot \vec{n}|}{\|\vec{n}\|}$$

展开点积：

$$\vec{QP} \cdot \vec{n} = A(x_0 - x_1) + B(y_0 - y_1) = (Ax_0 + By_0) - (Ax_1 + By_1)$$

因为 $Q$ 在直线上，满足 $Ax_1 + By_1 + C = 0$，即 $Ax_1 + By_1 = -C$，所以：

$$\vec{QP} \cdot \vec{n} = Ax_0 + By_0 - (-C) = Ax_0 + By_0 + C$$

代回投影公式，就得到经典的**点到直线距离公式**：

$$d = \frac{|Ax_0 + By_0 + C|}{\sqrt{A^2 + B^2}}$$

> 关键的"巧"就在于：$Q$ 满足直线方程这个条件，让 $Ax_1 + By_1$ 恰好等于 $-C$，从而把 $Q$ 的坐标完全消掉了——所以不管你在直线上选哪个 $Q$，结果都一样。

注意分母 $\sqrt{A^2 + B^2}$ 其实就是法向量 $\vec{n}$ 的长度 $\|\vec{n}\|$，这和 SVM 里的 $\frac{2}{\|w\|}$ 是极其相似的！

#### 总结

- **点与点的距离：** 直接算两点坐标差的平方和开根号（不需要法向量）
- **点与线、线与线、面与面的距离：** 只要想求垂直距离，法向量就是最强大的"投影神器"
...
SVM 的决策规则：$w \cdot x_i + b > 0$ → 预测正类（$+1$），$w \cdot x_i + b < 0$ → 预测负类（$-1$）。标签也是 $+1$ 和 $-1$，所以把标签和模型输出相乘，就能判断对错：

**分对的情况（乘积为正）：**

- 真实 $y_i = +1$，模型输出 $w \cdot x_i + b = +3$（预测正类，对了）：

$$y_i(w \cdot x_i + b) = (+1) \times (+3) = +3 > 0 \quad \checkmark$$

- 真实 $y_i = -1$，模型输出 $w \cdot x_i + b = -2$（预测负类，对了）：

$$y_i(w \cdot x_i + b) = (-1) \times (-2) = +2 > 0 \quad \checkmark$$

**分错的情况（乘积为负）：**

- 真实 $y_i = +1$，模型输出 $w \cdot x_i + b = -0.5$（预测负类，错了）：

$$y_i(w \cdot x_i + b) = (+1) \times (-0.5) = -0.5 < 0 \quad \times$$

- 真实 $y_i = -1$，模型输出 $w \cdot x_i + b = +1.5$（预测正类，错了）：

$$y_i(w \cdot x_i + b) = (-1) \times (+1.5) = -1.5 < 0 \quad \times$$

**规律：** 分对时 $y_i$ 和模型输出同号，乘积为正；分错时异号，乘积为负。

**为什么是 $\geq 1$ 而不是 $> 0$？** $> 0$ 只代表"分对了"，而 $\geq 1$ 代表"分对了**且离决策边界足够远**"（在间隔边界之外）。比如上面第一个例子乘积 $= 3 \geq 1$，说明这个点很安全；如果某个正类样本的模型输出只有 $+0.3$，乘积 $= 0.3$，虽然分对了但离边界太近，SVM 仍然对它施加惩罚，逼它远离边界。
...
想象一个小窗口（卷积核）在图像上从左到右、从上到下滑动，每到一个位置就和窗口覆盖的区域做加权求和，产生一个输出值：

```
输入图像 5×5          卷积核 3×3         输出的一个值

a b c d e             w1 w2 w3
f g h i j             w4 w5 w6           output = a·w1 + b·w2 + c·w3
k l m n o     →       w7 w8 w9                   + f·w4 + g·w5 + h·w6
p q r s t                                        + k·w7 + l·w8 + m·w9
u v w x y             ↑ 窗口覆盖左上角 3×3 区域
```

然后窗口往右滑一格，再算一个值；滑到头了换下一行，直到扫完整张图。
...
- **输入尺寸**：原图的宽/高（如 $32$）
- **卷积核尺寸**：小窗口的宽/高（如 $3$）
- **Padding**：在图像四周补零的圈数。补零是为了让边缘像素也能被窗口中心覆盖到，防止输出尺寸缩小
- **Stride**：窗口每次滑动的步长。$\text{Stride}=1$ 就是一格一格滑，$\text{Stride}=2$ 就是跳着滑（输出尺寸减半）

**例子：为什么尺寸不变？**

输入 $32 \times 32$，卷积核 $3 \times 3$，$\text{Padding}=1$，$\text{Stride}=1$：

$$\frac{32 - 3 + 2 \times 1}{1} + 1 = \frac{31}{1} + 1 = 32$$

直觉：$3 \times 3$ 的卷积核本来会让每边缩小 1 个像素（左边少 1、右边少 1），但 $\text{Padding}=1$ 恰好在每边补了 1 圈零，刚好补回来。这是最常用的搭配——**保持尺寸不变，让缩小的工作交给池化层去做**。
