...
### 代码阶段4
  ...
  原本模型用 16bit 存储，现在用 4bit，显存直接降到 1/4。
  - 比如 7B 模型：16bit 需要 ~14GB，4bit 只要 ~3.5GB
  ...
  用你准备好的指令-回答对数据集来训练。SFT = Supervised Fine-Tuning（监督微调），让模型学会按你的格式和风格回答问题。
  ...
  训练时：基础模型(几GB) + LoRA文件(几十MB)  → 分开存，方便换不同LoRA
...
LoRA 是一种**参数高效微调（PEFT）**技术，用于以极低的计算成本对大型预训练模型进行微调。
 ...
 假设模型中有一个权重矩阵 W，大小是 4×4：
  ...
  ---
  一句话总结
  LoRA = 冻结原始大模型，只训练一对很小的矩阵 A 和 B，它们的乘积就是对原模型的微调修正。
  这样你用一张普通显卡就能微调几十亿参数的大模型，而且效果接近全量微调。

#### 7B的参数 4bit 
4 bit = 0.5 Byte（1 Byte = 8 bit） 3.5GB
  主要参数来源：
...
是的，你的代码只训练了 0.68% 的参数，这是因为使用了 LoRA (Low-Rank         
  Adaptation)。                                                            
                                                                             
  配置在 llm_training_guide/llm_training_guide.py 的 LoRA 配置部分（约第
  210-226 行）：                                                             
...
### 代码阶段7
  ...
  把模型权重从 fp16（每个参数 2 字节）压缩到 4bit（每个参数 0.5 字节），显存占用降到约 1/4。比如 8B   
  模型从 ~16GB 降到 ~4.5GB，中低端显卡也能跑。代价是精度有少量损失，但对生成任务影响通常不大。        

  7b. KV Cache

  Transformer 生成第 N 个 token 时，attention 需要用到前面所有 token 的 Key 和 Value
  向量。如果不缓存，每生成一个新 token 都要重新算前面所有 token 的 K/V，计算量是 O(n²)。

  KV Cache 的做法：把已经算过的 K/V 存下来，每步只算新 token 的 K/V 然后追加到缓存里。这样每步计算量从
   O(n) 降到 O(1)，用空间换时间。代价是显存随序列长度线性增长。

  7c. 投机采样（Speculative Decoding）

  自回归生成的瓶颈是串行——每次只能生成一个 token。投机采样的原理：

  1. 用一个小模型（Qwen2-0.5B）快速连续生成 k 个候选 token
  2. 把这 k 个 token 一次性喂给大模型（Qwen2-1.5B）并行验证
  3. 大模型从前往后检查，接受概率匹配的 token，在第一个不接受的位置拒绝并用大模型自己的分布重新采样

  关键点：大模型验证 k 个 token 的成本和生成 1 个 token
  差不多（因为可以并行算），所以如果小模型猜对率高，就能实现 2~3x
  加速。而且数学上保证最终输出的分布和只用大模型生成完全一致，不损失质量。
...
⏺ 这段代码从三个维度评估模型质量：                                                                         
  ...
  原理： PPL = exp(平均loss)，值越低说明模型对文本预测越准确。                                             

  实现方式：
  - 加载 WikiText-2 测试集，拼接成一段长文本并 tokenize
  - 用滑动窗口（窗口 512，步长 256）遍历整段文本，每次取一段 input_ids 送入模型，用 labels=input_ids
  计算交叉熵损失
  ...
  PPL 就是模型在每个位置平均"犹豫"多少个候选词。 PPL=22.65 意味着模型平均在约 23
  个词之间犹豫。
  ...
  ROUGE-L 衡量的是预测文本与参考文本的最长公共子序列比例，越高说明生成内容与参考答案越接近。

  ---
  总结： 三个指标互补——PPL 看"语言能力"，标准基准看"知识/能力覆盖"，ROUGE-L 看"实际生成质量"。
