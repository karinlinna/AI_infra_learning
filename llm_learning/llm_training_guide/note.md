...
逐块拆解这段预训练代码
 ...
 ---
  ...
  loss = 5.2（交叉熵）
           ↓ 反向传播
  计算梯度，但【不更新】权重
  梯度存入累积缓冲区（÷8），因为显存不够所以分开算的，两个batch需要累加8次所以/8
  ...
  核心思路：用小 batch（省显存）+ 梯度累积（模拟大 batch）来达到训练效果和显存的平衡。每 8 个 micro-step
  才真正更新一次模型参数。
...
### 代码阶段4
  ...
  - 保存 LoRA 权重：只有几十 MB（不是完整模型）
  - 合并权重：将 LoRA 权重合并回基础模型（merge_and_unload），生成一个完整模型用于推理部署
